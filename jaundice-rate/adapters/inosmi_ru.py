...
def test_sanitize():
    resp = requests.get('https://inosmi.ru/20220618/politika-254580026.html')
    resp.raise_for_status()
    clean_text = sanitize(resp.text)

    assert 'Конфликт на Украине продолжается восемь лет.' in clean_text
    assert 'Благодаря борьбе народного ополчения с фашистами в Донбассе,' in clean_text
    ...
    assert 'Конфликт на Украине продолжается восемь лет.' in clean_plaintext
    assert 'Благодаря борьбе народного ополчения с фашистами в Донбассе,' in clean_plaintext
